...
ViT-B/16 基线模型完整训练脚本
特点：
1. 输入尺寸限制为 224x224 (为了避免高分辨率下的 OOM)。
2. 使用 AdamW 优化器 (ViT 标配)。
3. 混淆矩阵颜色为橙色 (Oranges)。
"""
...
# 1. 训练核心函数
# =========================================================================
def train_vit(data_dir, num_epochs=100, batch_size=16, patience=15):
    # ⚠️ [关键差异] ViT 必须降低分辨率，否则计算量爆炸
    # 如果你在这里强行改成 1280，99% 的概率会报 Out Of Memory
    img_size = 224
    checkpoint_path = 'vit_checkpoint.pth'
    best_model_path = 'vit_best.pth'

    print(f"\n[Baseline Config] ViT-B/16 | Size: {img_size}x{img_size} | Batch: {batch_size}")
    print("⚠️ 注意: 输入尺寸已设为 224x224 以适应 ViT 架构特性。")

    # --- 数据增强 ---
    data_transforms = {
        'train': transforms.Compose([
            transforms.Resize((img_size, img_size)),  # 强制缩放
    ...
    model = ViTBaseline(num_classes=2, pretrained=True)
    model = model.to(device)

    # 优化器: ViT 强烈建议使用 AdamW，SGD 很难训练动
            ...
            for phase in ['train', 'val']:
                ...
                if phase == 'val':
                    ...
                    if epoch_loss < best_loss:
                        print(f"  ★ ViT Loss Improved ({best_loss:.4f} -> {epoch_loss:.4f}). Saving BEST...")
...
# 2. 完整评估函数
# =========================================================================
def evaluate_comprehensive(model, data_dir, img_size=224):  # 注意这里默认也是 224
    print("\n" + "=" * 50)
    print(">>> 启动 ViT 全面评估...")
    print("=" * 50)
    ...
    df_report.to_csv('vit_report.csv', encoding='utf-8-sig')
    ...
    plt.title('混淆矩阵 - ViT (Baseline)', fontsize=14)
    plt.ylabel('真实标签', fontsize=12)
    plt.xlabel('预测标签', fontsize=12)
    plt.savefig('vit_confusion_matrix.png', dpi=300, bbox_inches='tight')
    plt.close()
    print("✅ 混淆矩阵已保存: vit_confusion_matrix.png")

    if error_records:
        df_errors = pd.DataFrame(error_records)
        df_errors.to_csv('vit_error_analysis.csv', index=False, encoding='utf-8-sig')
...
if __name__ == "__main__":
    ...
    if os.path.exists(DATA_DIR):
        # 训练 (ViT 输入小，batch_size 可以大一点，比如 16 或 32)
        trained_model = train_vit(DATA_DIR, num_epochs=150, batch_size=32, patience=30)

        # 评估 (保持 img_size=224)
        evaluate_comprehensive(trained_model, DATA_DIR, img_size=224)
    else:
