...
ResNet50 基线模型完整训练脚本
功能：
1. 训练 ResNet50 (Transfer Learning)。
2. 包含断点续训、早停机制。
3. 包含完整评估模块 (Blue Colormap 混淆矩阵)。
"""
...
try:
    ...
    print("✅ 成功导入 ResNet 模型")
except ImportError:
    print("❌ 致命错误: 找不到 resnet_model.py")
    sys.exit(1)
...
# 1. 训练核心函数
# =========================================================================
def train_resnet(data_dir, num_epochs=100, batch_size=4, patience=15):
    # ResNet50 处理 1280x1280 非常吃显存，建议 batch_size 不要太大
    img_size = 640
    checkpoint_path = 'resnet_checkpoint.pth'
    best_model_path = 'resnet_best.pth'

    print(f"\n[Baseline Config] ResNet50 | Size: {img_size}x{img_size} | Batch: {batch_size}")
    ...
    # Windows 下 num_workers 设为 0 以防多进程报错
    ...
    # 优化器: ResNet 经典搭配是 SGD，但为了收敛快，这里用 AdamW 也可以
    # 如果显存不够，可以尝试冻结 backbone 的前几层 (这里默认全参数微调)
            ...
            for phase in ['train', 'val']:
                ...
                if phase == 'val':
                    ...
                    if epoch_loss < best_loss:
                        print(f"  ★ ResNet Loss Improved ({best_loss:.4f} -> {epoch_loss:.4f}). Saving BEST...")
...
def evaluate_comprehensive(model, data_dir, img_size=1280):
    ...
    df_report.to_csv('resnet_report.csv', encoding='utf-8-sig')
    print("✅ ResNet 报告已保存: resnet_report.csv")
    ...
    plt.title('混淆矩阵 - ResNet50 (Baseline)', fontsize=14)
    plt.ylabel('真实标签', fontsize=12)
    plt.xlabel('预测标签', fontsize=12)
    plt.savefig('resnet_confusion_matrix.png', dpi=300, bbox_inches='tight')
    plt.close()
    print("✅ 混淆矩阵已保存: resnet_confusion_matrix.png")

    # (3) 坏例 CSV
    if error_records:
        df_errors = pd.DataFrame(error_records)
        df_errors.to_csv('resnet_error_analysis.csv', index=False, encoding='utf-8-sig')
        print(f"\n⚠️ 发现 {len(error_records)} 个错误样本，详情见 resnet_error_analysis.csv")
    else:
        print("\n🎉 ResNet 模型验证集全对！")
...
if __name__ == "__main__":
    ...
    if os.path.exists(DATA_DIR):
        # 1. 训练 (ResNet 较慢，batch_size 建议设小)
        trained_model = train_resnet(DATA_DIR, num_epochs=150, batch_size=16, patience=20)
